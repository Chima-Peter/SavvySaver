...
function SignUp() {
   ...
   const handleName = (name: string) => {
      if (!validator.isEmpty(name)) {
         if (!validator.isAlpha(name))
            setSignUpData({ ...signUpData, ['nameError']: `Name should contain only alphabets`})
         if (name.length < 3)
            setSignUpData({ ...signUpData, ['nameError']: `Name should be at least 3 characters`})
      } else 
         setSignUpData({ ...signUpData, ['nameError']: `Enter your name`})
   }
   ...
   const handleEmail = (email : string) => {
      if (!validator.isEmpty(email)) {
         ...
         setSignUpData({ ...signUpData, ['emailError']: `Enter email address`})
   }
   ...
   const handlePassword = (password: string) => {
      if (!validator.isEmpty(password)) {
         ...
         setSignUpData({ ...signUpData, ['passwordError']: `Enter password`})
   }
    ...
    <main className='w-full flex min-h-[100vh] gap-10 flex-col items-center font-main font-medium dark:bg-black ' >
      ...
      <section className='lg:border border-gray-300 lg:w-[800px] p-4 lg:shadow-lg lg:rounded-md md:border md:w-full md:shadow-md md:p-6 dark:border-0 md:py-10 flex gap-7 flex-col items-center justify-center'>
         <form className='flex flex-col gap-4' noValidate autoComplete='on' onSubmit={handleSubmit}>
               ...
               <p className='mt-2 self-center text-xs text-[#91919F]'>
