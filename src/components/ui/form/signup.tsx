      ...
      <section className='lg:border border-gray-300 lg:w-[800px] p-4 lg:shadow-lg lg:rounded-md md:border md:w-full md:shadow-md md:p-6 md:py-10 flex gap-7 flex-col justify-center'>
         <form className='flex flex-col gap-4' noValidate autoComplete='on' onSubmit={handleSubmit}>
               <label htmlFor="name" className='flex flex-col w-[100%]'>
                  ...
                  <span className='text-[10px] max-w-[380px] px-1 font-medium pt-1 text-red-600 '>{signUpData.nameError}</span>
               </label>
               <label htmlFor="email" className='flex flex-col w-[100%] text-sm text-black'>
                  ...
                  <span className='text-[10px] max-w-[380px] px-1 pt-1 font-medium text-red-600 '>{signUpData.emailError}</span>
               </label>
               <label htmlFor="password" className='flex flex-col w-[100%]'>
                 <div className='flex items-center relative'>
                  ...
                  <span className='text-[10px] max-w-[380px] px-1 pt-1 font-medium text-red-600 '>{signUpData.passwordError}</span>
               </label>
               <div  className='flex px-2 text-[11px] gap-1 items-start md:items-center font-medium tracking-tight'>
                  <input type="checkbox" name="checkbox" onChange={handleChange} id="check" className='border-4 cursor-pointer accent-[#7F3DFF] border-[#7F3DFF] outline-none rounded-lg w-4 h-4 lg:w-4 lg:h-4 translate-x-[-5%]' />
                  <p>
               ...
               <button type="submit" className="bg-[#7F3DFF] text-[#FCFCFC] border border-[#7F3DFF] rounded-xl text-sm font-bold py-3 px-4 mt-4  focus:outline-none  tracking-wider" 
               >
                  Sign Up
               </button>
               <p className='mt-2 text-xs text-[#91919F] text-center'>
                  Already have an account?&nbsp;
                   <Link to={'/login'} className='font-semibold text-[#7F3DFF] underline tracking-wider dark:font-medium'>
                     Login
                  </Link>
               </p>
         </form>
