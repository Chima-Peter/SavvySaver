      ...
      <section className='lg:border border-gray-300 dark:border-0 lg:w-[800px] p-4 lg:shadow-lg lg:rounded-md md:border md:w-full md:shadow-md md:p-6 md:py-10 flex gap-7 flex-col items-center justify-center'>
         <form className='flex flex-col gap-4' noValidate autoComplete='on' onSubmit={handleSubmit}>     
               <label htmlFor="email" className='flex flex-col w-[100%] text-sm text-black'>
                  <input autoFocus type="email" name='email' autoComplete='on' onBlur={emailBlur} onChange={handleChange} value={loginData.email} placeholder='Email' id='email' className='text-xs md:text-sm autofill:bg-clip-text text-black px-4 py-2 rounded-lg border placeholder:text-xs placeholder:text-[#91919F] border-gray-100 border-t-gray-200 shadow-md focus:outline-none focus:border-2 focus:border-[#F1F1FA] dark:autofill:fill-none dark:autofill:bg-clip-padding'/>
               ...
               <label htmlFor="password" className='flex flex-col relative'>
                  <div className='flex items-center relative'>
                     <input ref={passwordRef} type="password" name='password' autoComplete='on' onBlur={passwordBlur} onChange={handleChange} value={loginData.password} placeholder='Password' id='password' className='w-[100%] px-4 py-2 rounded-lg  text-xs md:text-sm autofill:bg-clip-text border placeholder:text-xs placeholder:text-[#91919F] border-gray-100 border-t-gray-200 shadow-md focus:outline-none focus:border-2 focus:border-[#F1F1FA] dark:autofill:fill-none dark:autofill:bg-clip-padding'/>
                     {
               ...
               <button type="submit" className="bg-[#7F3DFF] text-[#FCFCFC] border border-[#7F3DFF] rounded-xl text-sm font-bold py-3 px-4 hover:bg-[#EEE5FF] hover:border-[#7F3DFF] hover:text-[#7F3DFF] focus:bg-[#EEE5FF] focus:border-[#7F3DFF] translate-y-[-15px] focus:outline-none focus:text-[#7F3DFF] tracking-wide  dark:font-medium" 
               >
                  Login
               </button>
               <Link className='text-sm self-center font-bold text-[#7F3DFF] dark:font-semibold' to='/forgot_password'>
                  Forgot Password?
               </Link>
               <p className='self-center text-xs text-[#91919F]'>
               Don't have an account yet? <Link to={'/signup'} className='font-semibold text-[#7F3DFF] underline dark:font-medium tracking-wider'>Sign up</Link>
               </p>
         </form>
      </section>
    </main>
